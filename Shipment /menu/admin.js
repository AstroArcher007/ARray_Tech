...
(function () {
  ...
  const isSoldOutToggle = document.getElementById("isSoldOutToggle");
  ...
  function loadDishIntoForm(dish) {
    ...
    isSoldOutToggle.checked = Boolean(dish.isSoldOut);
  ...
  dishForm.addEventListener("submit", async (event) => {
    ...
    const isSoldOut = isSoldOutToggle.checked;
    ...
    const payload = { name, description, price, category, isSpecial, isAlcoholic, isSoldOut, dietType, spiceLevel, order };
  ...
  async function toggleSoldOut(id, currentValue) {
    try {
      await menuCollection.doc(id).update({ isSoldOut: !currentValue });
      showToast(!currentValue ? "Dish marked as sold out." : "Dish marked back in stock.");
    } catch (error) {
      console.error("Couldn't update sold-out status:", error);
      showToast("Couldn't update that dish. Please try again.", true);
    }
  }
  ...
  function renderDishCard(dish, draggable) {
    const soldOut = Boolean(dish.isSoldOut);
    return `
      <article class="dish-card${draggable ? " is-draggable" : ""}${soldOut ? " is-sold-out" : ""}" data-id="${dish.id}" ${draggable ? 'draggable="true"' : ""}>
        ...
        <div class="dish-card__main">
          <div class="dish-card__top">
            ...
            ${soldOut ? `<span class="dish-card__soldout">${ALERT_ICON}Sold out</span>` : ""}
          </div>
        ...
        <div class="dish-card__actions">
          <button class="icon-btn soldout-btn${soldOut ? " is-active" : ""}" title="${soldOut ? "Mark back in stock" : "Mark as sold out"}" aria-label="${soldOut ? "Mark" : "Mark"} ${escapeHtml(dish.name)} as ${soldOut ? "back in stock" : "sold out"}">${ALERT_ICON}</button>
  ...
  function wireDishCardButtons() {
    dishTable.querySelectorAll(".dish-card").forEach((card) => {
      ...
      card.querySelector(".soldout-btn").addEventListener("click", () => toggleSoldOut(id, Boolean(dish.isSoldOut)));
    });
  }
